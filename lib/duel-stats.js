  ...
  for (const duel of matching) {
    for (const leg of duel.legs || []) {
      ...
      for (const player of leg.players || []) {
        ...
        if (String(duel.mode || '') === 'elimination') {
          if (!Object.prototype.hasOwnProperty.call(current, 'finalScore')) current.finalScore = 0;
          if (!Object.prototype.hasOwnProperty.call(current, 'eliminations')) current.eliminations = 0;
          if (!Object.prototype.hasOwnProperty.call(current, 'eliminated')) current.eliminated = 0;
        }
        ...
        if (String(duel.mode || '') === 'elimination') current.finalScore = Number(player.scored || 0);
        ...
        if (String(duel.mode || '') === 'elimination') current.eliminations += Number(player.eliminations || 0);
        if (String(duel.mode || '') === 'elimination') current.eliminated += Number(player.eliminated || 0);
